"""
api_server.py — FastAPI + WebSocket bridge between the DIRAC bot and the dashboard.
...
DIRAC_CAPITAL_LIMIT = float(os.getenv("DIRAC_CAPITAL_LIMIT", "50"))
...
async def _cancel_then_place(signals: list[PipelineSignal]) -> None:
    ...
    # DIRAC HALT: block all trades if stop-loss triggered
    if _session_stop_triggered:
        logger.info("DIRAC HALT active — skipping all trades")
        return
    now = time.time()
...
async def _sync_live_balance() -> None:
    ...
    try:
        ...
        # DIRAC stop-loss: halt if loss >= capital limit or >= 20% of session start
        ...
        if total_loss >= DIRAC_CAPITAL_LIMIT or session_loss_pct >= 0.20:
            _session_stop_triggered = True
            logger.warning("DIRAC HALT: loss $%.2f (%.0f%%) — trading stopped",
...
async def _startup() -> None:
    ...
    logger.info("DIRAC API started. Bot running. Broadcaster running.")
...
async def dirac_status():
    return {
        ...
        "total_loss": _dirac_initial_balance - bot_state["balance"],
        "capital_limit": DIRAC_CAPITAL_LIMIT,
    }
